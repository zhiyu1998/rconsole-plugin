  ...
  async neteaseStatus (e, reck) {
    ...
    try {
      const statusResponse = await axios.get(statusUrl, {
        headers: {
          'User-Agent': COMMON_USER_AGENT,
          Cookie: reck || this.neteaseCookie
        }
      })
      ...
      if (images && images.length > 1) {
        const replyImagesPromises = images.map(async (item) => {
          try {
            ...
            return {
              message: segment.image(`base64://${base64Data}`),
              nickname: this.e.sender.card || this.e.user_id,
              user_id: this.e.user_id
            }
          } catch (error) {
            logger.error(`[R插件]图片处理失败：${item}`, error)
            return null
          }
        })

        // 等待所有图片处理完成
        Promise.all(replyImagesPromises).then((replyImages) => {
        // 过滤掉 null 值
          const validReplyImages = replyImages.filter((img) => img !== null)
          if (validReplyImages.length > 0) {
            e.reply(Bot.makeForwardMsg(validReplyImages))
          } else {
            e.reply('所有图片处理失败，请检查链接是否有效。')
          }
        })
      }
