                            ...
                            const cleanHtmlText = (html) => {
                                ...
                                return html
                                    .replace(/<a[^>]*?href="([^"]*?)"[^>]*?>(.*?)<\/a>/g, (match, href, text) => {
                                        ...
                                        if (cleanHref.startsWith('heybox://')) {
                                            try {
                                                const decodedHref = decodeURIComponent(cleanHref);
                                                const linkData = JSON.parse(decodedHref.replace('heybox://', ''));
                                                if (linkData?.link?.linkid) {
                                                    const linkId = linkData.link.linkid;
                                                    return `${formattedText} (https://www.xiaoheihe.cn/app/bbs/link/${linkId})`;
                                                }
                                            } catch (e) {
                            ...
                            .replace(/<a[^>]*?href="([^"]*?)"[^>]*?>(.*?)<\/a>/g, (match, href, text) => {
                                ...
                                if (cleanHref.startsWith('heybox://')) {
                                    try {
                                        ...
                                        if (linkData?.link?.linkid) {
                                            const linkId = linkData.link.linkid;
                                            return `\n${formattedText} (https://www.xiaoheihe.cn/app/bbs/link/${linkId})`;
                                        }
                                    } catch (e) {
                                        return formattedText;
                                    }
