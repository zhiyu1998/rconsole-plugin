...
export function startBBDown(videoUrl, downloadDir, BBDownOptions) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const command = `BBDown ${videoUrl} -e \"hevc,av1,avc\" -q \"720P 高清, 480P 清晰\" --work-dir ${downloadDir} ${biliSessData ? '-c SESSDATA=' + biliSessData : ''} ${pParam} -F temp --skip-subtitle --skip-cover ${biliUseAria2 ? '--use-aria2c' : ''} ${biliCDN ? '--upos-host ' + biliCDN : ''}`;
        logger.info(command);
        // logger.info(command);
